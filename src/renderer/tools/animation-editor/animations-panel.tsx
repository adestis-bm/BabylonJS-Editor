...
import {
    Button, NonIdealState, Divider, Card, Elevation, Tree, ITreeNode, Classes,
    IconName, ContextMenu, Menu, MenuItem, Tag,
...
export class AnimationsPanel extends React.Component<IAnimationPanelProps, IAnimationPanelState> {
    ...
    public render(): React.ReactNode {
        ...
        return (
            <Space direction="vertical" style={{ width: "100%" }}>
                <Card elevation={Elevation.ZERO} style={{ background: "#394b59" }}>
                    <h3><a>Actions</a></h3>
                    <Divider />
                    <Button text="Add..." small={true} fill={true} onClick={() => this._handleAddAnimation()} />
                </Card>
                <Card elevation={Elevation.ZERO} style={{ background: "#30404d" }}>
                    <h3><a>Animations</a></h3>
                    <Divider />
                    {noAnimation}
                    {animationsList}
                </Card>
    ...
    private _refreshAnimationsList(): ITreeNode<Animation>[] {
        if (!this.state.selectedAnimatable) { return []; }
        ...
        return this.state.selectedAnimatable.animations?.map((a, index) => ({
            id: index,
            label: a.name,
